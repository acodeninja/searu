//! Live gobuster directory brute-force against the CWE-22 lab: with a SecLists wordlist it discovers
//! /download, recorded as an endpoint observation. Ignored by default: needs the Docker daemon and
//! network, builds the gobuster image and downloads a wordlist. Run with
//! `cargo test -p searu --test gobuster_live -- --ignored`.
...
const CONTAINER: &str = "searu-gobuster-it";
const HOST_PORT: u16 = 5082;

const ROE: &str = r#"{
    "scope": { "targets": [
        { "type": "domain", "value": "localhost", "port": 5082 },
        { "type": "ip", "value": "127.0.0.1", "port": 5082 }
    ] },
    "allowed_techniques": ["T1595"]
}"#;

#[test]
#[ignore = "requires the docker daemon and network; builds the gobuster image and a wordlist"]
fn gobuster_discovers_the_download_route() {
    ...
    assert!(
        ...
        "gobuster did not discover /download; observations were:\n{observations}"
    );
}
